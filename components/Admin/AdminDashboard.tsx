    ...
    const fetchRealData = async () => {
        ...
        try {
            ...
            if (profiles) {
                ...
                // Calculate Metrics (simplified for client-side demo)
                ...
                setMetrics(prev => ({
                    ...
                    activeToday: Math.round((totalCount || 0) * 0.15),
                    totalMRR: mrr
                }));
...
const ActivityItem: React.FC<{ type: 'signup' | 'payment' | 'churn'; message: string; time: string }> = ({ type, message, time }) => {
    ...
    const Icon = icons[type]; // Fix: select based on type or use type directly if valid
